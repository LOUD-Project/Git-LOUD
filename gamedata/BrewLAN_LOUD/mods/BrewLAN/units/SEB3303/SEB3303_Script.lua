    ...
    OpenState = State() {
        ...
        Main = function(self)
            ...
            if not self.LightsOn then
                ...
                --Fix for FAF changing the bone names.
                local bone = 'ConstuctBeam'
				
                if import(import( '/lua/game.lua' ).BrewLANPath() .. '/lua/legacy/VersionCheck.lua').VersionIsFAF() then
                    bone = 'ConstructBeam'
                end
				
                self.Trash:Add(CreateAttachedEmitter(self, bone .. '01',army, '/effects/emitters/light_red_rotator_01_emit.bp'):ScaleEmitter( 2.00 ))
                self.Trash:Add(CreateAttachedEmitter(self, bone .. '02',army, '/effects/emitters/light_red_rotator_01_emit.bp'):ScaleEmitter( 2.00 ))
                self.LightsOn = true
