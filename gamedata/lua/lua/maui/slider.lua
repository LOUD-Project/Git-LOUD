    ...
    __init = function(self, parent, isVertical, startValue, endValue, thumb, thumbOver, thumbDown, background, debugname)
        ...
        if self._isVertical then
            ...
            self._thumb.Bottom:Set(function()
                    return LOUDFLOOR(self.Bottom() - (((self._currentValue() - self._startValue) / (self._endValue - self._startValue)) * (self.Bottom() - (self.Top() + self._thumb.Height()))))
                end)
        else
            LayoutHelpers.AtVerticalCenterIn(self._thumb, self)
            self._thumb.Left:Set(function()
                    return LOUDFLOOR(self.Left() + (((self._currentValue() - self._startValue) / (self._endValue - self._startValue)) * (self.Right() - (self.Left() + self._thumb.Width()))))
                end)
        end
...
IntegerSlider = Class(Slider)
{
    __init = function(self, parent, isVertical, startValue, endValue, indentValue, thumb, thumbOver, thumbDown, background)
        ...
        self._indentValue = LOUDFLOOR(indentValue)
    end,
    
    _Constrain = function(self,value)
        value = Slider._Constrain(self,value)
        value = LOUDFLOOR(value / self._indentValue) * self._indentValue
        return value
    end,
}
