...
Text = Class(moho.text_methods, Control) {
    ...
    OnInit = function(self)
        Control.OnInit(self)
        self.Height:Set(function() return LOUDFLOOR(self.FontAscent() + self.FontDescent()) end)
    ...
    SetClipToWidth = function(self, clipToWidth)
        if clipToWidth then
            ...
            self.Width:Set(function() return LOUDFLOOR(self.TextAdvance()) end)
        end
    ...
    StreamText = function(self, text, speed)
        ...
        self.OnFrame = function(self, deltaTime)
            ...
            if STR_Utf8Len(goalText) > STR_Utf8Len(self:GetText()) then
                self:SetText(STR_Utf8SubString(goalText, 1, LOUDFLOOR(self.ElapsedTime * speed)))
            else
...
function FitText(text, lineWidth, advanceFunction)
    ...
    if textLen > 1 then
        for ch = 2, textLen do
            ...
            if LOUDCEIL(advanceFunction(sub)) >= lineWidthFunc(curLine) then
    ...
    for packedWord in string.gfind(text, "[^ \t]+") do
        ...
        if lfStartIndex then
            ...
            while lfStartIndex do
                if lfStartIndex - curIndex > 0 then
                    ...
                    LOUDINSERT(words, LOUDSUB(packedWord, curIndex, lfStartIndex - 1))
                end
        ...
        for index, word in words do
            ...
            if wordWidth > lineWidthFunc(table.getsize(result)) then
                ...
                local charsPerLine = LOUDFLOOR(STR_Utf8Len(word) / wordLines)
