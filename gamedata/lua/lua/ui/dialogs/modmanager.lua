...
local function CreateDependsDialog(parent, text, yesFunc)
    ...
    LayoutHelpers.SetHeight(dialog, 300)
...
local function CreateSavePresetDialog(parent, modStatus)
    ...
    dialog.Width:Set(background.Width)
    LayoutHelpers.SetHeight(dialog, 60)
	...
	nameEdit.Width:Set(function() return background.Width() - LayoutHelpers.ScaleNumber(80) end)
...
local function ModConfigDialog(parent, modInfo, config, modStatusEntry)
    ...
    -- Give title a rectangle to float in the centre of, so that font size
    -- changes don't make it shrink upwards
    local titleBox = Group(dialog)
    LayoutHelpers.AtTopIn(titleBox, dialog.border.tm, 12)
    LayoutHelpers.AtHorizontalCenterIn(titleBox, dialog)
    titleBox.Width:Set(dialog.Width())
    LayoutHelpers.SetHeight(titleBox, 30)
    local fontSize = math.floor((LayoutHelpers.ScaleNumber(145) / string.len(modInfo.name)) + LayoutHelpers.ScaleNumber(10))
    local title = UIUtil.CreateText(titleBox, "Mod Settings: "..modInfo.name, fontSize, UIUtil.titleFont)
    LayoutHelpers.AtCenterIn(title, titleBox)
    ...
    optionContainer.Height:Set(dialog.Height() - LayoutHelpers.ScaleNumber(32))
    ...
    local function CreateOptionElement(index)
        ...
        LayoutHelpers.SetHeight(optionList[index], 46)
        ...
        LayoutHelpers.SetWidth(optionList[index].combo, 240)
...
function CreateDialog(over, inLobby, exitBehavior, useCover, modStatus)
    ...
    LayoutHelpers.SetDimensions(modListContainer, 380, 455)
    ...
    local function CreateModGroup(i)
        ...
        LayoutHelpers.SetHeight(grp, 36)
        ...
        grp.icon = Bitmap(grp)
        LayoutHelpers.SetDimensions(grp.icon, 36, 36)
    ...
    modListContainer.CalcVisible = function(selfMLC)
        ...
        for bi = 1, table.getsize(folderOrder) do
            ...
            if skip <= 0 then
                ...
                modListTable[i].name:SetFont('Arial', 20)
            ...
            for _, uid in block.uids do
                ...
                modListTable[i].name:SetFont('Arial', 14)
    ...
    LayoutHelpers.SetWidth(modDetails.author, 300)
    ...
    LayoutHelpers.SetHeight(modDetails.desc, 46)
    ...
    LayoutHelpers.SetHeight(modDetails.extras, 192)
    modDetails.extras.Width:Set(modDetails.Width() - LayoutHelpers.ScaleNumber(40))
    ...
    LayoutHelpers.SetHeight(modDetails.copyright, 28)
    ...
    local saveBtn = UIUtil.CreateButtonStd(panel, '/widgets/tiny', "Save", 12)
    ...
    local disableBtn = UIUtil.CreateButtonStd(panel, '/scx_menu/small-btn/small', "Disable All", 16)
