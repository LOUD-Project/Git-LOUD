...
local function CreateNameFilter(data)
    ...
    if data.choices and data.choices[1] and table.getn(data.choices) > 6 then
        LayoutHelpers.SetHeight(group, 30 + math.floor((table.getn(data.choices) - 1)/6) * 25)
    else
    ...
    if data.choices then
		...
		LayoutHelpers.SetDimensions(group.edit, 200, 15)
...
function CreateDialog(x, y)
	...
	LayoutHelpers.SetDimensions(dialog, 600, 720)
	...
	LayoutHelpers.SetDimensions(count, 50, 15)
    ...
    local function CreateElementMouseover(unitData,x,y)
        ...
        mouseover.Left:Set(x + LayoutHelpers.ScaleNumber(20))
        mouseover.Top:Set(y + LayoutHelpers.ScaleNumber(20))
        mouseover.Height:Set(function() return mouseover.img.Height() + LayoutHelpers.ScaleNumber(4) end)
        mouseover.Width:Set(function() return mouseover.img.Width() + math.max(mouseover.name.Width(), mouseover.desc.Width()) + LayoutHelpers.ScaleNumber(8) end)
