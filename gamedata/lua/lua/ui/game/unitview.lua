...
local GetUnitIconPath       = import('/lua/ui/game/gamecommon.lua').GetUnitIconPath
...
local function FormatTime(seconds)
    return LOUDFORMAT("%02d:%02d", LOUDFLOOR(seconds/60), LOUDFLOOR(math.mod(seconds, 60)))
end
...
local statFuncs = {
	...
	function(info)
	    ...
	    local massUsed = LOUDMAX(info.massRequested, info.massConsumed)

	    if info.massProduced > 0 or massUsed > 0 then
	        local netMass = info.massProduced - massUsed
	        local absNetMass = LOUDABS(netMass)
	        local str

	        if absNetMass < 5 then
	            str = LOUDFORMAT('%+.3g', netMass)
	        elseif absNetMass < 10 then
	            str = LOUDFORMAT('%+.2g', netMass)
	        elseif absNetMass < 100 then
	            str = LOUDFORMAT('%+.3g', netMass)
	        else
	            str = LOUDFORMAT('%+d', LOUDROUND(netMass))
	        end

	        return str, UIUtil.UIFile('/game/unit_view_icons/mass.dds'), '00000000'
	...
	function(info)
	    ...
	    local energyUsed = LOUDMAX(info.energyRequested, info.energyConsumed)

	    if info.energyProduced > 0 or energyUsed > 0 then
	        local netEnergy = info.energyProduced - energyUsed
	        local absNetEnergy = LOUDABS(netEnergy)

	        if absNetEnergy < 10 then
	            return LOUDFORMAT('%+.2g', netEnergy)
	        elseif absNetEnergy < 100 then
	            return LOUDFORMAT('%+.3g', netEnergy)
	        else
	            return LOUDFORMAT('%+d', LOUDROUND(netEnergy))
	        end
	    else
	        return false
	    end
	...
	function(info)
        ...
        -- FAF has a different veteran mechanic based upon tracking the mass value of what's been killed
	...
	function(info)
	    if info.kills > 0 then
	        return LOUDFORMAT('%d', info.kills)
	    else
	        return false
	    end
	end,
	...
	function(info)
	    ...
	    if info.tacticalSiloMaxStorageCount > 0 or info.nukeSiloMaxStorageCount > 0 then
	        ...
	        if info.userUnit:IsInCategory('VERIFYMISSILEUI') then
	            ...
	            local curEnh = GetEnhancements(info.userUnit:GetEntityId())

	            if curEnh then
	                if curEnh.Back == 'TacticalMissile' or curEnh.Back == 'Missile' then
	                    return LOUDFORMAT('%d / %d', info.tacticalSiloStorageCount, info.tacticalSiloMaxStorageCount) , 'tactical'
	                elseif curEnh.Back == 'TacticalNukeMissile' then
	                    return LOUDFORMAT('%d / %d', info.nukeSiloStorageCount, info.nukeSiloMaxStorageCount), 'strategic'
	                else
	                    return false
	                end
	            else
	    ...
	    elseif info.userUnit and not table.empty(GetAttachedUnitsList({ info.userUnit })) then
	        return LOUDFORMAT('%d', LOUDGETN(GetAttachedUnitsList({ info.userUnit }))), 'attached'
	    else
	        return false
	    end
	end,
...
function UpdateWindow(info)
    ...
    if info.blueprintId == 'unknown' then
        ...
        for index = 1, LOUDGETN(controls.statGroups) do
            local i = index
        ...
        for index = 1, LOUDGETN(statFuncs) do
            local i = index
        ...
        if info.health then
            ...
            controls.health:SetText(LOUDFORMAT("%d / %d", info.health, info.maxHealth ))
        else
            controls.healthBar:Hide()
        end
        ...
        if info.focus then
            ...
            local path, valid = GetUnitIconPath(__blueprints[info.focus.blueprintId])
            ...
            if info.focus.health and info.focus.maxHealth then
                ...
                controls.actionText:SetText(LOUDFORMAT('%d%%', (info.focus.health / info.focus.maxHealth) * 100))
        ...
        elseif info.focusUpgrade then
            ...
            controls.actionText:SetFont(UIUtil.bodyFont, 14)
            controls.actionText:SetText(LOUDFORMAT('%d%%', info.workProgress * 100))
            controls.actionIcon:Show()
            controls.actionText:Show()
        ...
        if Prefs.GetOption('uvd_format') == 'full' and bp.Display.Abilities then
            ...
            local i = 1
            local maxWidth = 0
            local index = LOUDGETN(bp.Display.Abilities)
            ...
            while bp.Display.Abilities[index] do
                ...
                maxWidth = LOUDMAX(maxWidth, controls.abilityText[i].Width())
                index = index - 1
                i = i + 1
...
function UpdateEnhancementIcons(info)
    ...
    if unit then
        existingEnhancements = GetEnhancements(unit:GetEntityId())
    end
...
function CreateUI()
    ...
    controls.bg.OnFrame = function(self, delta)
        ...
        if not info and import("/lua/gaz_ui/modules/selectedinfo.lua").SelectedInfoOn then
            ...
            if selUnits and LOUDGETN(selUnits) == 1 and import('/lua/ui/game/unitviewDetail.lua').View.Hiding then
        ...
        elseif self:GetAlpha() > 0 then
            self:SetAlpha(LOUDMAX(self:GetAlpha() - (delta*3), 0), true)
        end
    end
