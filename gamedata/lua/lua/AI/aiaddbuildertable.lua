...
-- this is where the builders (platoons) are added to the builder managers (EM,FM,PFM)
 
-- just note - later on in the AddBuilder process I'll use the faction filter
-- so that platoon definitions that have the FactionIndex tag will only get loaded
-- if the brain is the same FactionIndex
function AddBuilderTable(aiBrain, locationType, builderTable)
