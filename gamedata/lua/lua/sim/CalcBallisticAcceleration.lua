...
CalculateBallisticAcceleration = function (weapon, proj, ProjectilesPerOnFire)
	...
	if bomber_table[entityId] then
		...
		local v_launcher = LOUDSQRT(math.pow(ux,2) + math.pow(uy,2) + math.pow(uz,2))
		local vhorz_launcher = LOUDSQRT(math.pow(ux,2) + math.pow(uz,2))
		...
		local dist = LOUDSQRT( math.pow(pos_target[1] -  pos_proj[1], 2) +  math.pow(pos_target[3] -  pos_proj[3], 2) )
		...
		local offset = (ProjectilesPerOnFire - 1) * 0.1 * (vhorz_launcher * 0.5)	--/ 2
		...
		local vhorz = LOUDSQRT( math.pow(vx-wx, 2) + math.pow(vz-wz, 2) )
