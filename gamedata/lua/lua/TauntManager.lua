...
function CreateTauntManager(name, fileName)
    ...
    local tManager = {
        ...
        AddPlayerIntelUnitTaunt = function(self, diagData, unit)
            local currNum, callback = self:TauntBasics(diagData)
            TriggerFile.CreateArmyIntelTrigger(callback, ArmyBrains[1], 'LOSNow', unit, true, categories.ALLUNITS, true, unit.Brain )
        end,

        AddIntelUnitTaunt = function(self, diagData, unit, lookingBrain)
            local currNum, callback = self:TauntBasics(diagData)
            TriggerFile.CreateArmyIntelTrigger(callback, lookingBrain, 'LOSNow', unit, true, categories.ALLUNITS, true, unit.Brain )
        end,
