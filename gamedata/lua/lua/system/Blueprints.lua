...
local bpcount = table.getn(__blueprints)
local count = 0
    ...
    for id, bp in __blueprints do
        ...
        if id ~= bp.BlueprintId then
            ...
            --local ist = tostring(bp)
            --local kst = tostring(__blueprints[bp.BlueprintId])
          
            --_ALERT(ist == kst and "Good Dupe" or "Bad Dupe", id, bp.BlueprintId, ist, kst)
    ...
    LOG("*AI DEBUG __Blueprints initialized with "..bpcount)
    LOG("*AI DEBUG removed "..count.." blueprints")
...
LOG("*AI DEBUG logging duplicate ID - used " .. ( (memstart*1024) ) .. " bytes")
