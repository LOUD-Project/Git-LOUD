...
URL0001 = Class(OldURL0001) {  
	...
	OnDamage = function(self, instigator, amount, vector, damageType)
		if not self.IsHunkering then 
			OldURL0001.OnDamage(self, instigator, amount, vector, damageType)
		end
	end,
	
	OnScriptBitSet = function(self, bit)
		OldURL0001.OnScriptBitSet(self, bit)
		...
		if bit == 7 then
			if self.canHunker then 
				...
				ForkThread(function()
					...
					local Hunker = CreateUnit('CYBRAN_HShield', self:GetArmy(), loc[1], loc[2], loc[3], ori[1], ori[2], ori[3], ori[4], 'Land')
