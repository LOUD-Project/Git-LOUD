UnitList = {
   
    T3ArmoredAssault = {
        UEF = {'BRNT3ABB', 50},		-- Ironfist
        Aeon = {'BROT3HM', 50},		-- Mogul 
        Cybran = {'BRMT3BM2', 15},	-- Dervish
		Seraphim = {'BRPT3BOT', 50}, -- Thaam-Thuum
    },
   
    T3LandArtillery = {
        Cybran = {'BRMT3ML', 40},	-- Pavestone
		Seraphim = {'BRPT3ML', 40},	-- Heth-Zthuha
    },
    
    T3LandBot = {
        UEF = {'BRNT3BT', 75},	-- Bull
        Aeon = {'BROT3BT', 20},		-- Transoma	
        Cybran = {'BRMT3BM2', 15},	-- Dervish
		Seraphim = {'BRPT3BOT', 25},	-- Thaam-Thuum
    },
	
	T3Amphibious = {
        UEF = {'BRNT3ABB', 50},		-- Ironfist
        Aeon = {'BROT3BT', 100},	-- Transoma	
        Cybran = {'BRMT3BM2', 35},	-- Dervish
		Seraphim = {'BRPT3BOT', 35}, -- Thaam-Thuum
	},
	
    T3LandAA = {
        Cybran = {'BRMT3LZT', 10},	-- Trilobyte
    },
	
	T3AmphibiousAA = {
        Cybran = {'BRMT3LZT', 100},	-- Trilobyte
    },

	

    T2LandDFTank = {
        Cybran = {'BRMT2MEDM', 40},	-- Immortal
		UEF = {'BRNT2BM', 40},		-- Banshee
        Aeon = {'BROT2ASB', 40},	-- Amphorak
		Seraphim = {'BRPT2BTBOT', 40},	-- Iltha
    },

    T2AttackTank = {
        UEF = {'BRNT2MT', 50},		-- Statue Rapid Assault
        Cybran = {'BRMT2HT', 50},	-- Hippo
    },
    
    -- December 22, 2020 -- T2 Land Amphibious does not exist in Default Factory Platoons.
    -- T2LandAmphibBot and T2LandAmphibTank exist.
    -- This should be split up into said Categories above.
    -- This explains why we would see pure platoons of Wagners, Riptides, etc.

    T2LandAmphibTank = {
        Aeon = {'BROT2ASB', 20},	-- Amphorak
		Cybran = {'BRMT2MEDM', 20},	-- Immortal
        UEF = {'BRNT2BM', 20},		-- Banshee
        Seraphim = {'BRPT2BTBOT', 20},	-- Iltha
    },

    T2LandAmphibBot = {
        Aeon = {'BROT2ASB', 40},	-- Amphorak
		Cybran = {'BRMT2MEDM', 40},	-- Immortal
        UEF = {'BRNT2BM', 40},		-- Banshee
        Seraphim = {'BRPT2BTBOT', 40},	-- Iltha
    },




    T1LandDFTank = {
        Aeon = {'BROT1BT', 35}, -- Hervour
		Seraphim = {'BRPT1HT', 70},		-- Yenshavoh
    },

    T1LandDFBot = {
        Aeon = {'BROT1EXM1', 50}, -- Bugbear
        Cybran = {'BRMT1EXM1', 50},	-- Proton
		Seraphim = {'BRPT1EXM1', 50},	-- Othazyne
    },
    
    T1LandArtillery = {
        Aeon = { 'BROT1ML', 40}, -- Wavecrest
    },

}