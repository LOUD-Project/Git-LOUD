...
float4 CalculateLighting( float3 inNormal, float3 inViewPosition, float3 inAlbedo, float specAmount, float waterDepth, float4 inShadow, uniform bool inShadows)
{
    ...
    light = light * 0.9;
    float3 powlight = (light.r + light.g + light.b) / 3;
    light = light * powlight;
...
float4 TerrainPS( VS_OUTPUT inV, uniform bool inShadows ) : COLOR
{
  ...
  float dotLightNormal = dot(SunDirection, normal);
    ...
    float4 outColor = CalculateLighting( normal, inV.mTexWT.xyz, albedo.xyz, 1-albedo.w, waterDepth, inV.mShadow, inV.mShadow);
    //outColor = float4(inV.mTexWT.rgb, 0);
	return outColor;
}
