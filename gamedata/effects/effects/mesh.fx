...
///
///
float4 AeonCloakPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    if ( 1 == mirrored ) clip(vertex.depth);
    float3x3 rotationMatrix = float3x3( vertex.binormal, vertex.tangent, vertex.normal);
...
float4 AeonCloakOverlayPS( NORMALMAPPED_VERTEX vertex) : COLOR0
{   
    // Diffuse texture
    float4 texcoord = vertex.texcoord0;
    texcoord.y += vertex.material.x * 0.00162;
    ...
    float4 dotLightNormal = saturate(dot(sunDirection,normal));
    float3 reflection = normalize( 2 * dotLightNormal * normal - normalize(sunDirection));
    float4 specular = pow( saturate( dot(reflection, vertex.viewDirection )), 128);
...
///
///
float4 CybranCloakPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    if ( 1 == mirrored ) clip(vertex.depth);
    float3x3 rotationMatrix = float3x3( vertex.binormal, vertex.tangent, vertex.normal);
...
///
///
float4 SeraphimCloakPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    if ( 1 == mirrored ) clip(vertex.depth);
    float3x3 rotationMatrix = float3x3( vertex.binormal, vertex.tangent, vertex.normal);
...
float4 CybranCloakLoFiPS( VERTEXNORMAL_VERTEX vertex) : COLOR0
{
...
float4 AeonCloakLoFiPS( VERTEXNORMAL_VERTEX vertex) : COLOR0
{
    float4 albedo = tex2D( albedoSampler, vertex.texcoord0);
    float4 specular = tex2D( specularSampler, vertex.texcoord0);
    albedo.rgb = lerp( vertex.color.rgb * vertex.material.y * 0.25, albedo.rgb, saturate(1 - specular.a) );
...
float4 SeraphimCloakLoFiPS( VERTEXNORMAL_VERTEX vertex) : COLOR0
{
    float4 albedo = tex2D( albedoSampler, vertex.texcoord0);
    float4 specular = tex2D( specularSampler, vertex.texcoord0);
    albedo.rgb = lerp( vertex.color.rgb * vertex.material.y * 0.25, albedo.rgb, saturate(1 - specular.a) );
...
/// CybranCloakOverlayPS
///
///
float4 CybranCloakOverlayPS( VERTEXNORMAL_VERTEX vertex) : COLOR0
{
    float4 secondary = tex2D(secondarySampler, vertex.texcoord0.xy );
    float4 alphamask1 = tex2D(secondarySampler, vertex.shadow.xy );
    float4 color = float4( secondary.a * 0.75, 0, 0, alphamask1.r * secondary.a );
...
///
///
float4 UEFCloakHiFiPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    if ( 1 == mirrored ) clip(vertex.depth);
    float3x3 rotationMatrix = float3x3( vertex.binormal, vertex.tangent, vertex.normal);
...
float4 UEFCloakLoFiPS( VERTEXNORMAL_VERTEX vertex) : COLOR0
{
    float4 albedo = tex2D( albedoSampler, vertex.texcoord0);
    float4 specular = tex2D( specularSampler, vertex.texcoord0);
    albedo.rgb = lerp( vertex.color.rgb * vertex.material.y * 0.25, albedo.rgb, saturate(1 - specular.a) );
...
///
///
float4 UEFCloakOverlayHiFiPS( VERTEXNORMAL_VERTEX vertex) : COLOR0
{
    float4 xshift = tex2D( secondarySampler, vertex.texcoord0.xy );
    float4 yshift = tex2D( secondarySampler, vertex.texcoord0.zw );
...
float4 UEFCloakOverlayLoFiPS( VERTEXNORMAL_VERTEX vertex) : COLOR0
{
    float4 xshift = tex2D( secondarySampler, vertex.texcoord0.xy );
    float4 yshift = tex2D( secondarySampler, vertex.shadow.xy );
...
///
///
float4 AeonCloakOverlayPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    if ( 1 == mirrored ) clip(vertex.depth.x);
...
/// SeraphimCloakOverlayPS
///
///
float4 SeraphimCloakOverlayPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    if ( 1 == mirrored ) clip(vertex.depth.x);
...
float4 UEFCloakPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    if ( 1 == mirrored ) clip(vertex.depth);
    float3x3 rotationMatrix = float3x3( vertex.binormal, vertex.tangent, vertex.normal);
...
///
///
float4 TransparentGlowPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    if ( 1 == mirrored ) clip(vertex.depth);
    float3x3 rotationMatrix = float3x3( vertex.binormal, vertex.tangent, vertex.normal);
...
///
///
float4 TransparentPS( NORMALMAPPED_VERTEX vertex, uniform bool hiDefShadows) : COLOR0
{
    if ( 1 == mirrored ) clip(vertex.depth);
    float3x3 rotationMatrix = float3x3( vertex.binormal, vertex.tangent, vertex.normal);
...
///
///
technique UEFCloak_HighFidelity
<
    string abstractTechnique = "UEFCloak";
    int fidelity = FIDELITY_HIGH;
...
>
{
    pass P0
    {
		...
		VertexShader = compile vs_1_1 NormalMappedVS();
        	PixelShader = compile ps_2_a UEFCloakHiFiPS(true);
    ...
    pass P1
    {
		AlphaState( AlphaBlend_SrcAlpha_InvSrcAlpha_Write_RGBA )
           ...
           VertexShader = compile vs_1_1 EffectVertexNormalHiFiVS( 16.0, 8.0, 0.0192, 0.0176, -0.0122, -0.0122 );
           PixelShader = compile ps_2_0 UEFCloakOverlayHiFiPS();
    }
}

technique UEFCloak_MedFidelity
<
    string abstractTechnique = "UEFCloak";
    int fidelity = FIDELITY_MEDIUM;
...
>
{
    pass P0
    {
		AlphaState( AlphaBlend_SrcAlpha_InvSrcAlpha_Write_RGB )
	      ...
	      VertexShader = compile vs_1_1 NormalMappedVS();
            PixelShader = compile ps_2_a UEFCloakPS(true);
    }
}

technique UEFCloak_LowFidelity
<
    string abstractTechnique = "UEFCloak";
    int fidelity = FIDELITY_LOW;
...
>
{
    pass P0
    {
		AlphaState( AlphaBlend_SrcAlpha_InvSrcAlpha_Write_RGB )
            ...
            VertexShader = compile vs_1_1 VertexNormalVS();
            PixelShader = compile ps_2_0 UEFCloakLoFiPS();
    }
}
...
///
///
technique CybranCloak_HighFidelity
<
    string abstractTechnique = "CybranCloak";
    int fidelity = FIDELITY_HIGH;
...
>
{
    pass P0
    {
        ...
        VertexShader = compile vs_1_1 NormalMappedVS();
        PixelShader = compile ps_2_a CybranCloakPS(true);
    }
    pass P1
    {
        AlphaState( AlphaBlend_SrcAlpha_InvSrcAlpha_Write_RGBA )
        ...
        VertexShader = compile vs_1_1 EffectVertexNormalLoFiVS( 14, 4, 0, 0, -0.008, 0.008 );
        PixelShader = compile ps_2_0 CybranCloakOverlayPS();
    }
}

technique CybranCloak_MedFidelity
<
    string abstractTechnique = "CybranCloak";
    int fidelity = FIDELITY_MEDIUM;
...
>
{
    pass P0
    {
        ...
        VertexShader = compile vs_1_1 NormalMappedVS();
	  PixelShader = compile ps_2_0 CybranCloakPS(false);
    }
}

technique CybranCloak_LowFidelity
<
    string abstractTechnique = "CybranCloak";
    int fidelity = FIDELITY_LOW;
...
>
{
    pass P0
    {
		...
		VertexShader = compile vs_1_1 VertexNormalVS();
		PixelShader = compile ps_2_0 CybranCloakLoFiPS();
    }
}
...
technique AeonCloak_HighFidelity
<
    string abstractTechnique = "AeonCloak";
    int fidelity = FIDELITY_HIGH;
...
>
{
    pass P0
    {
		...
		VertexShader = compile vs_1_1 NormalMappedVS();
		PixelShader = compile ps_2_a AeonCloakPS(true);
    ...
    pass P1
    {
		...
		PixelShader = compile ps_2_0 AeonCloakOverlayPS(false);

    }
}

technique AeonCloak_MedFidelity
<
    string abstractTechnique = "AeonCloak";
    int fidelity = FIDELITY_MEDIUM;
...
>
{
    pass P0
    {
		...
		VertexShader = compile vs_1_1 NormalMappedVS();
		PixelShader = compile ps_2_0 AeonCloakPS(false);
    }
}

technique AeonCloak_LowFidelity
<
    string abstractTechnique = "AeonCloak";
    int fidelity = FIDELITY_LOW;
...
>
{
    pass P0
    {
		...
		VertexShader = compile vs_1_1 VertexNormalVS();
		PixelShader = compile ps_2_0 AeonCloakLoFiPS();
    }
}
...
technique SeraphimCloak_HighFidelity
<
    string abstractTechnique = "SeraphimCloak";
    int fidelity = FIDELITY_HIGH;
...
>
{
    pass P0
    {
		...
		VertexShader = compile vs_1_1 NormalMappedVS();
		PixelShader = compile ps_2_a SeraphimCloakPS(true);
    }
    pass P1
    {
        AlphaState( AlphaBlend_SrcAlpha_InvSrcAlpha_Write_RGBA )
        ...
        PixelShader = compile ps_2_0 SeraphimCloakOverlayPS(false);
    }
}

technique SeraphimCloak_MedFidelity
<
    string abstractTechnique = "SeraphimCloak";
    int fidelity = FIDELITY_MEDIUM;
...
>
{
    pass P0
    {
		...
		VertexShader = compile vs_1_1 NormalMappedVS();
		PixelShader = compile ps_2_0 SeraphimCloakPS(false);
    }
}

technique SeraphimCloak_LowFidelity
<
    string abstractTechnique = "SeraphimCloak";
    int fidelity = FIDELITY_LOW;
...
>
{
    pass P0
    {
		...
		VertexShader = compile vs_1_1 VertexNormalVS();
		PixelShader = compile ps_2_0 SeraphimCloakLoFiPS();
    }
}

///-------------------------------------------------------------------------------------------

technique TransparentGlow
<
    string abstractTechnique = "TransparentGlow";

    string cartographicTechnique = "CartographicBuild";    
    string depthTechnique = "Depth";
    int renderStage = STAGE_DEPTH + STAGE_PREWATER + STAGE_PREEFFECT;

    ///Set the transparancy here.    
    int parameter = 0.35;
>
{
    pass P0
    {
        ...
        VertexShader = compile vs_1_1 NormalMappedVS();
        PixelShader = compile ps_2_a TransparentGlowPS(true);
    }
    pass P1
    {
        AlphaState( AlphaBlend_SrcAlpha_InvSrcAlpha_Write_RGBA )
...
<
    string abstractTechnique = "Transparent";
...
>
{
    pass P0
    {
        ...
        VertexShader = compile vs_1_1 NormalMappedVS();
        PixelShader = compile ps_2_a TransparentPS(true);
    }
}

technique Invisable
<
    string abstractTechnique = "Invisable";
...
technique UnitFormationPreview
<
    string abstractTechnique = "UnitFormationPreview";
...
technique TDF_MedFidelity
<
    string abstractTechnique = "TDF";
    int fidelity = FIDELITY_MEDIUM;
...
>
{
    ...
    pass P0
    {
        ...
        VertexShader = compile vs_1_1 ClutterVS();
	    PixelShader = compile ps_2_0 ShieldPS();
        VertexShader = compile vs_1_1 ShieldPositionNormalOffsetVS( 0.02, 3,3,3,1, 0.01,0, -0.003,0, 0,-0.02, 0,-0.001);
        
    }
}

technique TDF_LowFidelity
<
    string abstractTechnique = "TDF";
    int fidelity = FIDELITY_LOW;
...
float4 PhasePS( EFFECT_VERTEX vertex ) : COLOR
{
    if ( 1 == mirrored ) clip(vertex.depth);
    ...
    {
        if( specular.b >= albedo.g )
            color.a = ( color.b >= normal.b ) ? 0.12 : lerp( 0.05, 0, sin(frac( 0.01 * time) * 1.14) );
        else
            color.a = ( normal.b >= albedo.r ) ? 0.2 : lerp( 0.01, 0.1, sin(frac( 0.01 * time) * 1.14) );
    }
    else
    {
        if( specular.r >= albedo.r )
            ...
            color.a = ( specular.g >= albedo.g ) ? 0.02 : lerp( 0.37, 0.46, sin(frac( 0.01 * time) * 1.14) );
    }
    ...
    // Adjust color of shield based on its health percentage
    float4 colorMod1 = lerp(float4( 0.5, 0.0, 0.0, 0.05 ), color, 0.5);
    colorMod1 = lerp( color, colorMod1 + color, sin(frac( 0.06 * time) * 1.14) );
    color = lerp( colorMod1, color, vertex.material.y );
    color += (colorMask.b * 0.72);

    // Add in our alpha channel to mask UV pinching at the top of the sphere
    color.a *= colorMask.a;// * 0.10;

    return color;
}

technique Phasing_MedFidelity
<
    string abstractTechnique = "Phasing";
    int fidelity = FIDELITY_MEDIUM;
...
>
{
    pass P0
    {
		...
		VertexShader = compile vs_1_1 FourUVTexShiftScaleVS( 1, 3, 16, 12, -0.001, 0, 0, 0, 0, 0, 0, 0 );
        	PixelShader = compile ps_2_0 PhasePS();
    }
}
...
float4 PhaseSeraphimPS( EFFECT_VERTEX vertex ) : COLOR
{
    if ( 1 == mirrored ) clip(vertex.depth);
...
technique PhasingSeraphim_MedFidelity
<
    string abstractTechnique = "PhasingSeraphim";
    int fidelity = FIDELITY_MEDIUM;
