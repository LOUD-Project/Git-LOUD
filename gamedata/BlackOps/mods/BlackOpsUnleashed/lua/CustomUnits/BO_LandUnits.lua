UnitList = {
	T1LandDFTank = {
		Seraphim = {'bsl0106', 10}, 		-- Protector Bot
	},

	T2AttackTank = {
		UEF = {'bel0211', 20 },     		-- Flamethrower
        Aeon = {'bal0206', 50 },    		-- Zealot
        Cybran = {'brl0205', 10 },  		-- Ambush Bot
		Seraphim = {'bsl0206', 10 },		-- Imp. Protector Bot
	},
    
    T3LandAA = {
        UEF = { 'belk002', 100 },			-- Cougar
        Aeon = { 'balk003', 100 },			-- Redeemer
        Cybran = { 'brlk001', 100 },		-- Bouncer
        Seraphim = { 'bslk004', 100 },		-- Lightning Tank
    },
    
    T3LandArtillery = {
        UEF = { 'bel0308', 20 },    		-- Bangerhead
    },
    
    T3LandArtilleryElite = {
        Seraphim = {'bsl0007', 100}, 		-- Elite T3 Artillery
    },
    
    T3LandBotElite = {
        Seraphim = {'bsl0005', 100}, 		-- Elite Siege Tank
    },
    
    T3ArmoredAssault = {
    	UEF = {'xel0307', 100 },     		-- Juggernaut
    	Aeon = {'bal0310', 100 },	   		-- Wraith Hovertank
    },
	
	T3Amphibious = {
    	UEF = {'xel0307', 50 },     		-- Juggernaut
    	Aeon = {'bal0310', 100 },	   		-- Wraith Hovertank
	},
    
    T3LandBot = {
    	Cybran = {'brl0307', 20 },  		-- Hailfire
    	Seraphim = {'bsl0310', 100},    	-- Lambda Bot        
    },
	
	T4LandExperimental1 = {
		Aeon = {'bal0402', 50},				-- Genesis Hover Exp	
		Seraphim = {'bsl0406', 15},		    -- Exp Protector	
	},
   
	T4LandExperimental2 = {
		Aeon = {'bal0403', 25},				-- Interrogator (old Inquisitor)		
	},
	
	T4LandExperimental3 = {
		UEF = {'bel0402', 50},	 			-- Goliath Mk. 2
		Aeon = {'bal0401', 20},				-- Inquisitor (new)		
		Cybran = {'brl0401', 75},			-- Basilisk
		Seraphim = {'bsl0401', 100},		-- Yenzotha Hover Exp
	},

}