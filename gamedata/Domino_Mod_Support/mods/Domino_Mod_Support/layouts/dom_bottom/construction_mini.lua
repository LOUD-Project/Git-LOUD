...
function SetLayout()
    ...
    controls.constructionGroup.Top:Set(function() return controlClusterGroup.Top() + 12 end) --construction, templates, enhancements panel
    controls.constructionGroup.Bottom:Set(controlClusterGroup.Bottom)
	
    if ordersControl then
        controls.constructionGroup.Left:Set(function() return ordersControl.Right() - 273 end) -- -16
    else
    ...
    controls.choices.Top:Set(function() return controls.minBG.Top() + 5 end) -- + 5
    controls.choices.Height:Set(50)
    controls.choices:SetToVertical(false)
    controls.choices.Left:Set(function() return controls.minBG.Left() + 26 end) -- + 26
    controls.choices.Right:Set(function() return controls.maxBG.Right() - 25 end) -- - 25
    ...
    LayoutHelpers.AtLeftTopIn(controls.choicesBGMin, controls.choices, -46) -- -46
    controls.choicesBGMax:SetTexture(UIUtil.UIFile('/game/construct-panel/que-panel_bmp_r.dds'))
    LayoutHelpers.AtRightTopIn(controls.choicesBGMax, controls.choices, -43) -- -43
    ...
    LayoutHelpers.AtLeftTopIn(controls.secondaryChoicesBGMin, controls.secondaryChoices, -46) -- -46
    ...
    LayoutHelpers.AtLeftIn(controls.secondaryProgress, controls.secondaryChoices, 5) -- 5
    LayoutHelpers.AtTopIn(controls.secondaryProgress, controls.secondaryChoices, 42) -- 42
    ...
    controls.extraBtn2:SetNewTextures(textures.midBtn.up, textures.midBtn.selected, textures.midBtn.over, 
        textures.midBtn.over, textures.midBtn.dis, textures.midBtn.dis)
    LayoutHelpers.AtLeftTopIn(controls.extraBtn1, controls.minBG, 10, 31) -- 10, 31
    LayoutHelpers.Below(controls.extraBtn2, controls.extraBtn1, 1) -- 1
    ...
    if table.getsize(controls.tabs) > 0 then
        for id, control in controls.tabs do
            ...
            if not prevControl then
                LayoutHelpers.AtLeftTopIn(control, controls.minBG, 82, 0) -- 82 , 0
            else
                local offset = 0
        ...
        LayoutHelpers.AtLeftIn(controls.minBG, controls.constructionGroup, 70)
...
function OnTabChangeLayout(type)
    ...
    if type != 'selection' then
        controls.choices.Left:Set(function() return controls.minBG.Left() + 78 end) -- + 85
        controls.choices.Right:Set(function() return controls.maxBG.Right() - 49 end) -- -49
		

		-------------------------------------------------------------------------
		local Ordercontrols = import('/lua/ui/game/orders.lua').controls
		Ordercontrols.bg:SetTexture(UIUtil.SkinnableFile('/game/orders-panel/order-panels_bmp.dds'))
		LayoutHelpers.AtLeftIn(Ordercontrols.bg, Ordercontrols.controlClusterGroup, 75)
		LayoutHelpers.AtBottomIn(Ordercontrols.bg, Ordercontrols.controlClusterGroup, 138)
		-------------------------------------------------------------------------
    ...
    if type == 'construction' or type == 'templates' then
        ...
        controls.choices.Top:Set(function() return controls.minBG.Top() + 32 end)
        LayoutHelpers.AtLeftTopIn(controls.extraBtn1, controls.minBG, 8, 30) -- 10, 31
	...
	-------------------------------------------------------------------------
		...
		Ordercontrols.bg:SetTexture(UIUtil.SkinnableFile('/game/orders-panel/order-panels_bmp.dds'))
		LayoutHelpers.AtLeftIn(Ordercontrols.bg, Ordercontrols.controlClusterGroup, 75)
		LayoutHelpers.AtBottomIn(Ordercontrols.bg, Ordercontrols.controlClusterGroup, 117)
	-------------------------------------------------------------------------
        ...
        controls.choices.Top:Set(function() return controls.minBG.Top() + 4 end) -- construction panel icons bar
        LayoutHelpers.AtLeftTopIn(controls.extraBtn1, controls.minBG, 4, 5)
        LayoutHelpers.Below(controls.extraBtn2, controls.extraBtn1, 1) -- pause button height.
        controls.choices.Left:Set(function() return controls.minBG.Left() + 75 end)
