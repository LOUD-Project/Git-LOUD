...
--local uimod= import(sca_utils .. 'uimod.lua')
local gamemain = import('/lua/ui/game/gamemain.lua')
local LayoutHelpers = import('/lua/maui/layouthelpers.lua')
local Prefs = import('/lua/user/prefs.lua')
local Group = import('/lua/maui/group.lua').Group
local Bitmap = import('/lua/maui/bitmap.lua').Bitmap
local pathuiimg='/mods/hotstats/textures'
local skinpath = '/mods/hotstats/skins'
...
function GetActualSkinFile()
	return skinpath..'/sca.skin'
end

function start_skinning_process()
	code={}
	import(skinpath.. '/sca.skin').updateBorders()
end
...
function reset_obj(object)
	local empty=pathuiimg.. '/empty_sca.dds'
	object.part=0
...
function Set_Group_Texture(list,object,parent,path, grouptype, offsetTable, centered)
	...
	--if not(parent) then parent:SetTexture(pathuiimg.. '/empty_sca.dds') end
	--if not(object) or object.Type!='Group'then --TEMP REMOVED
	if not object then
		object = Group(parent) 
